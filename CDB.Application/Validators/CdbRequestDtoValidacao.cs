...
{
    public static class CdbRequestDtoValidacao
    {
        public static bool CdbRequestDtoValidar(CdbRequestDto cdbRequestDto)
        {
            if (cdbRequestDto.ValorInicial <= 0.01M || cdbRequestDto.ValorInicial > 250000)
                throw new ArgumentException("Valor Inicial deve ser maior que zero e menor ou igual a 250.000,00.");

            if (cdbRequestDto.QtdMeses <= 2 || cdbRequestDto.QtdMeses > 60)
